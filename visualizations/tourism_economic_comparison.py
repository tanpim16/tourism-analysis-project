...
df_share = (
    df.groupby(['date','City_type_EN'])['total_visitors']
    .sum()
    .reset_index()
)
...
# filter study period
df_share = df_share.query("'2023-01-01' <= date <= '2025-12-01'")

# ─── Prepare stacked structure ────────────────────────────────────────────────
pivot = (
    df_share.pivot(index='date',
                   columns='City_type_EN',
                   values='visitor_share')
    .sort_index()
)

dates = pivot.index
major = pivot['Major City']
secondary = pivot['Secondary City']

os.makedirs('visualizations', exist_ok=True)

# ─── Layout (Header + Chart Area) ─────────────────────────────────────────────
...
ax.plot(dates, major, color='white', linewidth=1.5)
ax.plot(dates, major + secondary, color='#333', linewidth=1)

# ─── Axis Formatting ──────────────────────────────────────────────────────────
ax.set_ylim(0, 1)
ax.yaxis.set_major_formatter(PercentFormatter(1.0))
ax.set_ylabel("Visitor Share")
...
print("✅ Figure 3 (100% stacked area) created successfully")